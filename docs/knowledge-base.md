...
Before the Studio binding, exact-target production refresh job `55ce619889c94b2b8a7e7f890c5fcf0d` validated the same four current/ongoing public cards with the deterministic adapter. The subsequent Studio verification returned those same four canonical URLs. After deployment, production job `95bf4c4b9b954088ae135e7d5c0de0e1` completed with collector `c_mt61hvcq1d8np500ya`, four normalized rows, and pipeline label `scraper-studio+public-card-enrichment`. The public registry reports the binding as `ready`, and all four rows appear in US, India, UK, and Singapore feeds. Treat all counts as time-specific. Online and hybrid cards carry explicit `GLOBAL` eligibility through normalization. Sanitized evidence lives under `scrapper-run-results/wemakedevs/`.
...
The live frontend release on 2026-08-24 is `8a230dbe378d4e4fd707544937891c6ea0018a34`; the backend release is `e9c2439`. GitHub quality run `32652525100` passed for the earlier `32d34fe` backend release; the `e9c2439` check was still pending at production verification time. Public verification returned HTTP 200 for the frontend and API health, with 40 worldwide rows. The four production WeMakeDevs rows are available in every supported country feed, and between one and three of them appear in each prize-ranked top ten at the verification time. Earlier browser verification covered desktop and 390×844 layouts, card-wide and explicit-button opening, modal content, the independent source URL, Escape dismissal, backdrop dismissal, scroll locking, and focus restoration. The card-wide hit target is implemented by stretching the existing semantic detail button rather than making the article a nested interactive control.
