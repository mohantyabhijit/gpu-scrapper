...
function usage(): never {
  throw new Error("usage: node --experimental-strip-types scripts/check-source-health.ts --source <slug> --collector <c_id> --input-url <https-url> --required-field <field> --before <path> --consumer <path> [--consumer <path>] [--repo-root <path>] [--output <path>]");
}

function parseArgs(argv: readonly string[]): Arguments {
  const values: Partial<Arguments> = { consumer: [], repoRoot: process.cwd() };
