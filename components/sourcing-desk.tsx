...
export default function SourcingDesk({ catalogBlob, visibleOfferIds, marketLabel, children }: { catalogBlob: string; visibleOfferIds: readonly string[]; marketLabel: string; children: ReactNode }) {
  ...
  function persist(next: SourceDeskOffer[]) {
    const bounded = next.slice(0, SOURCE_DESK_LIMIT);
    setSelected(bounded);
    setPending(null);
    try { window.localStorage.setItem(SOURCE_DESK_STORAGE_KEY, serializeSourceDesk(next)); } catch { /* browser limitation */ }
    setStatus("");
  }
  function add(offer: SourceDeskOffer) {
    ...
    if (result.pending) { setPending(result.pending); setOpen(true); setStatus(`This replaces the ${selected[0].market} / ${selected[0].currency} desk. Acknowledgement is required.`); return; }
  ...
  function replaceDesk() { if (pending) { const next = pending; const result = selectSourceDeskOffer(selected, next, true); persist(result.selected); setStatus(`Desk replaced with ${next.market}. Prices remain in ${next.currency}; no cross-market total was made.`); } }
