...
import { buildSourcingBrief, canonicalizeStored, isSafeUrl, serializeSourceDesk, SOURCE_DESK_LIMIT, type SourceDeskOffer } from "./sourcing-desk-model";

export { buildSourcingBrief, canonicalizeStored, serializeSourceDesk, SOURCE_DESK_LIMIT } from "./sourcing-desk-model";
...
export default function SourcingDesk({ catalogBlob, marketLabel, fixture, children }: { catalogBlob: string; marketLabel: string; fixture: boolean; children: ReactNode }) {
  ...
  function add(offer: SourceDeskOffer) {
    ...
    if (selectedIds.has(canonical.id)) { persist(selected.filter((item) => item.id !== canonical.id)); return; }
    if (selected.length > 0 && selected[0].market !== canonical.market) { setPending(canonical); setStatus(`This replaces the ${selected[0].market} desk. Acknowledgement is required.`); return; }
    ...
    persist([...selected, canonical]);
  }
  function remove(offer: SourceDeskOffer) { persist(selected.filter((item) => item.id !== offer.id)); }
  function replaceDesk() { if (pending) { const next = pending; persist([next]); setPending(null); setStatus(`Desk replaced with ${next.market}. Prices remain in ${next.currency}; no cross-market total was made.`); } }
