...
import SourcingDesk, { type SourceDeskOffer } from "../components/sourcing-desk";
...
export default async function Home({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return <main className="site-shell">
    ...
    <section className="offers-section" id="offers"><div className="section-heading"><div><p className="eyebrow"><span>02</span> / COMPARISON DESK</p><h2>Offers worth opening.</h2></div><p className="section-note">Market-local comparison · {marketInfo.label}<br />{liveRead ? "Exact observed timestamps per row" : "Explicit fixture timestamps"}</p></div>
      ...
      <SourcingDesk offers={filteredOffers.map(({ id, market: offerMarket, model, brand, source, currency, price, availability, observedAt, healthState, freshness, productUrl }): SourceDeskOffer => ({ id, market: offerMarket, model, brand, source, currency, price, availability, observedAt, healthState, freshness, productUrl }))} marketLabel={marketInfo.label} fixture={!liveRead} />
      <div className="offer-grid">{filteredOffers.map((offer) => <article className="offer-card" key={offer.id}><div className="offer-topline"><span className={`signal signal-${offer.freshnessTone}`}><span /> {offer.availability}</span><time className="freshness" dateTime={offer.observedAt}>{offer.freshness}</time></div><div className="card-gpu-art" aria-hidden="true"><span>{offer.model.split(" ").at(-1)}</span><div /></div><div className="offer-content"><p className="offer-model">{offer.model}</p><h3>{offer.brand}</h3><p className="offer-spec">{offer.vram} <span>·</span> {marketInfo.label}</p><div className="offer-price-row"><strong>{formatPrice(offer.price, offer.currency, marketInfo.locale)}</strong><span className={`collector-badge health-${offer.healthState}`}>{liveRead ? `${offer.healthState} source` : "fixture source"}</span></div><div className="offer-footer"><span className="source-name"><span className={`source-avatar source-${slugify(offer.source)}`}>{offer.source.slice(0, 1)}</span>{offer.source}</span><span className="offer-actions"><Link href={`/gpu/${offer.modelSlug}?market=${market}`}>Compare details <span aria-hidden="true">→</span></Link><a href={offer.productUrl} target="_blank" rel="noreferrer">Verify at retailer <span aria-hidden="true">↗</span></a></span></div></div></article>)}</div>{filteredOffers.length === 0 && <div className="empty-state"><strong>No matching offers.</strong><p>Try another model, retailer, or clear the active filters.</p><Link className="button button-primary" href={`/?market=${market}#offers`}>Show all offers</Link></div>}</section>
