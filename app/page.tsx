...
export default async function Home({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return <main className="site-shell">
    ...
    <section className="offers-section" id="offers"><div className="section-heading"><div><p className="eyebrow"><span>02</span> / COMPARISON DESK</p><h2>Offers worth opening.</h2></div><p className="section-note">Market-local comparison · {marketInfo.label}<br />{liveRead ? "Observed timestamps shown per row" : "Fixture window: today"}</p></div>
      <form className="filter-bar" method="get" action="#offers"><label className="market-field"><span className="sr-only">Market</span><select name="market" defaultValue={market}>{Object.values(markets).map((item) => <option key={item.code} value={item.slug}>{item.label} · {item.currency}</option>)}</select></label><label className="search-field"><span aria-hidden="true">⌕</span><span className="sr-only">Search offers</span><input name="q" defaultValue={valueOf(params.q) ?? ""} placeholder="Search model, brand, retailer…" /></label><label><span className="sr-only">Availability</span><select name="availability" defaultValue={availability}><option value="all">All signals</option><option value="in-stock">In stock</option><option value="low-stock">Low stock</option><option value="stale-check">Stale check</option></select></label><label><span className="sr-only">Retailer</span><select name="source" defaultValue={source}><option value="all">All retailers</option>{sources.map((item) => <option key={item} value={item.toLowerCase().replaceAll(" ", "-")}>{item}</option>)}</select></label><button className="button filter-button" type="submit">Filter</button></form>
