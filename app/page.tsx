...
export default async function Home({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const market = getMarket(valueOf(params.market));
  ...
  return <main className="site-shell">
    ...
    <section className="offers-section" id="offers"><div className="section-heading"><div><p className="eyebrow"><span>02</span> / COMPARISON DESK</p><h2>Offers worth opening.</h2></div><p className="section-note">Market-local comparison · {marketInfo.label}<br />{liveRead ? "Observed timestamps shown per row" : "Fixture window: today"}</p></div>
      <form className="filter-bar" method="get" action="#offers"><label className="market-field"><span className="sr-only">Market</span><select name="market" defaultValue={market}><option value="us">United States · USD</option><option value="uk">United Kingdom · GBP</option><option value="india">India · INR</option><option value="singapore">Singapore · SGD</option></select></label><label className="search-field"><span aria-hidden="true">⌕</span><span className="sr-only">Search offers</span><input name="q" defaultValue={valueOf(params.q) ?? ""} placeholder="Search model, brand, retailer…" /></label><label><span className="sr-only">Availability</span><select name="availability" defaultValue={availability}><option value="all">All signals</option><option value="in-stock">In stock</option><option value="low-stock">Low stock</option><option value="stale-check">Stale check</option></select></label><label><span className="sr-only">Retailer</span><select name="source" defaultValue={source}><option value="all">All retailers</option>{sources.map((item) => <option key={item} value={item.toLowerCase().replaceAll(" ", "-")}>{item}</option>)}</select></label><button className="button filter-button" type="submit">Filter</button></form>
      ...
      <div className="offer-grid">{filteredOffers.map((offer) => <article className="offer-card" key={offer.id}><div className="offer-topline"><span className={`signal signal-${offer.freshnessTone}`}><span /> {offer.availability}</span><span className="freshness">{offer.freshness}</span></div><div className="card-gpu-art" aria-hidden="true"><span>{offer.model.split(" ").at(-1)}</span><div /></div><div className="offer-content"><p className="offer-model">{offer.model}</p><h3>{offer.brand}</h3><p className="offer-spec">{offer.vram} <span>·</span> {marketInfo.label}</p><div className="offer-price-row"><strong>{formatPrice(offer.price, offer.currency)}</strong><span className="collector-badge">{liveRead ? "live row" : "fixture offer"}</span></div><div className="offer-footer"><span className="source-name"><span className={`source-avatar source-${offer.source.toLowerCase().replaceAll(" ", "-")}`}>{offer.source.slice(0, 1)}</span>{offer.source}</span><a href={offer.productUrl} target="_blank" rel="noreferrer">Verify at retailer <span aria-hidden="true">↗</span></a></div></div></article>)}</div>{filteredOffers.length === 0 && <div className="empty-state"><strong>No matching offers.</strong><p>Try a model name, retailer, or reset the filters.</p><Link className="button button-primary" href={`/?market=${market}#offers`}>Show all offers</Link></div>}</section>
