...
export default async function GpuDetail({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="site-shell detail-page"><div className="demo-banner" role="note"><span className="pulse-dot" aria-hidden="true" /><strong>{catalogLabel}</strong><span>{catalogMessage}</span></div><header className="topbar"><Link href="/" className="brand"><span className="brand-mark" aria-hidden="true">R</span><span>raster<span className="brand-dot">.</span></span></Link><nav className="main-nav" aria-label="Primary navigation"><Link href={`/?market=${market}#offers`}>Compare offers</Link><Link href="/how-it-works">How it works</Link><Link href="/data-health">Data health</Link></nav><span className="status-chip"><span className="status-dot" aria-hidden="true" /> {liveRead ? "d1 view" : "fixture view"}</span></header><div className="breadcrumbs"><Link href={`/?market=${market}`}>Raster · {marketInfo.label}</Link><span>/</span><span>{model.name}</span></div><section className="detail-hero"><div className="detail-art card-gpu-art"><span>{model.name.split(" ").at(-1)}</span><div /></div><div><p className="eyebrow"><span>{liveRead ? "MODEL DATA" : "MODEL FIXTURE"}</span> / {modelOffers.length} LOCAL SOURCES</p><h1>{model.name}</h1><p className="detail-spec">{model.vram} · {marketInfo.label} · {marketInfo.currency} comparison</p><div className="detail-price"><span>{liveRead ? "Lowest normalized row in this market" : "Lowest fixture in this market"}</span><strong>{formatPrice(lowest.price, lowest.currency)}</strong><small>{lowest.source} · {lowest.freshness}</small></div></div></section><section className="detail-offers" aria-label={`${model.name} ${marketInfo.label} ${liveRead ? "normalized rows" : "fixtures"}`}><div className="section-heading"><div><p className="eyebrow"><span>OFFER LEDGER</span> / MARKET-LOCAL</p><h2>Compare board partners.</h2></div><p className="section-note">No cross-market ranking<br />No currency conversion</p></div><div className="detail-table">{modelOffers.map((offer) => <div className="detail-row" key={offer.id}><div><strong>{offer.brand}</strong><span>{offer.source} · {offer.vram}</span></div><div className="detail-signal"><span className={`signal signal-${offer.freshnessTone}`}><span /> {offer.availability}</span><small>{offer.freshness}</small></div><strong className="detail-row-price">{formatPrice(offer.price, offer.currency)}</strong><a href={offer.productUrl} target="_blank" rel="noreferrer">Verify ↗</a></div>)}</div></section><footer className="site-footer"><Link href={`/?market=${market}`} className="brand"><span className="brand-mark" aria-hidden="true">R</span><span>raster<span className="brand-dot">.</span></span></Link><span>{liveRead ? "D1 normalized snapshot · verify before purchase" : "Fixture snapshot · verify before purchase"}</span><Link href="/how-it-works">How it works ↗</Link></footer></main>;
}
